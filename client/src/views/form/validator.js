...
const validate = (formData) => {
  ...
  if (!regNumber.test(formData.heightMin))
    errors.heightMin = "Value is not valit";
  if (!regNumber.test(formData.heightMax))
    errors.heightMax = "Value is not valit";
  if (!regNumber.test(formData.weightMin))
    errors.weightMin = "Value is not valit";
  if (!regNumber.test(formData.weightMax))
    errors.weightMax = "Value is not valit";
  if (!regNumber.test(formData.life))
    errors.life = "Value is not valit";
  ...
  if (Number(formData.weightMin) >= Number(formData.weightMax))
    errors.weightMax = "Minimum cannot be greater than or equal to maximum";
  ...
  if (!formData.life) errors.life = "Life span is required";

  if (!formData.temperaments || formData.temperaments.length === 0)
    errors.temperaments = "Not selected any value yet";
  return errors;
};
